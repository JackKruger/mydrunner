...
Rollover is intentionally a real risk on slopes and at-speed turns into ruts — it's tuned to be controllable on the road but punishing off it. Grip is contact-load and slip driven; chassis pitch must never create hidden traction.
...
- `packages/shared/src/physics/ruts.ts` — sparse, session-only 0.25 m rut field in quantized 16×16 tiles. Owners predict bounded elliptical stamps, the room validates and globally sequences them, and late joiners receive chunked tile sync. Wheel support samples the field without rebuilding the coarse Rapier heightfield.
...
- `markerCatalog.ts` — `MARKER_INFO`: `SURFACE_INFO`'s pattern applied to markers. Kind union, palette label, default/limit radius, whether `yaw` means anything, and the colour the bay and its editor overlay are painted in — one table read by the decoder, the editor palette and the client's marker meshes alike. `garageBay` is the only kind the simulation reads (`Room` leases bays by marker id); the rest ride along for the cargo objective.
...
- `packages/client/src/input.ts` / `touchInput.ts` — keyboard (+ handbrake toggle) and on-screen analog steer pad / pedals; both merge in `sampleInput()`. The aux buttons are two groups, not one row: a trail set that is always on screen and a pit set (starter, reset, tyre pressure, radio, mute, full screen) behind `#aux-more-btn`. Thirteen at once walled off the top of a landscape phone. **The brake pedal and the reverse request are one physical control**: the touch UI ships two pedals and no reverse button, so the brake pedal also feeds negative throttle, and Shift+S pairs the same two on the keyboard. Issuing both at once deadlocks the truck — the service brake locks all four wheels while the box sits in reverse at full torque, so it never moves and the tacho parks at its stall target — which is why `sampleInput()` resolves them into one command against `setForwardSpeed()`: brake while still rolling forward, then release for the gear once stopped. That signed speed (`LocalSimulation.forwardSpeed()`, pushed per fixed step from `main.ts`) is the only vehicle state the input layer reads, and it must stay signed — a magnitude cannot say which way the truck is going.
...
- `packages/client/src/markers.ts` — `MarkerMeshes`: what an authored marker looks like. The garage bay paints itself from its own marker — slab, outline, chevrons, wheel stop and a floating chevron beacon, all sized from the marker's trigger radius, so the paint and the volume you have to be inside are the same number. It replaced three white stripes hardcoded into `landmarks.ts` against `STATION.parking`, which lined up with the shipped bays by coincidence and left a bay authored anywhere else invisible. Cosmetic only: **no colliders**, because a wheel stop you could actually hit would change what "drive fully into the bay" means on every existing map. `preview` (editor) draws every kind plus its trigger ring; the game draws only `garageBay`, the one kind it simulates.
...
- `editor/tools.ts` — tool ids, defaults, keyboard bindings. The paint palette derives from `SURFACE_INFO`, the object palette from `OBJECT_INFO` and the marker palette from `MARKER_INFO`, so a new surface or kind appears without anyone remembering to add it. `applyKindDefaults` reseeds the dimension sliders from the kind — one global 1.6 / 2 used to size a traffic cone like a boulder — and `applyMarkerKindDefaults` does the same for marker radius. `cursorRadius` is why the brush ring shows a bay's own 1.65 m rather than the 12 m sculpt brush.
...
- `packages/client/src/playerUI.ts` — the in-world HUD: speed, tacho, gear + transfer-case sticks, damage and engine-status readouts, control guide. The bottom-centre panels (dev diagnostics, engine status, transmission, cluster) live in one flex column, `.hud-dock` — they were four absolutely positioned elements with hand-tuned offsets restated in three media queries, which is how the cluster came to draw its drivetrain and tyre-pressure lines *through* its own bottom border and off the viewport. **Position panels relative to the dock, not the viewport.** On touch the transmission panel joins that column and collapses to its header (`.expanded` / `body.shifter-open`), because the H-gate is a quarter of a landscape phone; the collapsed header carries the gear and range readout, since the gates are the only other place either is shown. Touch layouts also run every HUD panel translucent (`--ui-panel-touch`) and pinned to an edge — the dock to the bottom-left in portrait, the aux tray to a corner block rather than a row across the horizon, chat to the free corner — because on a phone every panel is drawn over the only view the player has.
- `packages/client/src/fullscreen.ts` — the full-screen toggle, offered as an aux-tray button and a menu setting. On a phone the browser chrome owns more of the screen than the whole HUD does, so this is a layout feature. Every registered control reads its state back from `fullscreenchange` rather than from its own clicks — the browser leaves full screen on its own (ESC, a system gesture) and a control that trusted what it asked for would then show the opposite of what the screen is doing. Where the API is missing (iPhone Safari offers it on `<video>` only) the controls remove themselves.
...
- **`@hotloop` means allocation-free, and a test enforces it.** Marking a function or method `@hotloop` puts it under `hotLoopAllocation.test.ts`, a TypeScript-AST guard that rejects object/array literals, `new`, spreads, inline callbacks and the allocating array helpers anywhere in its body. Its reach is the marked body's own syntax: it cannot see into callees, and it cannot see Rapier's own returns (`castShape`, `translation()` and friends each hand back a fresh JS object, which is most of the ~4.7 KB/tick still allocated). So a hot function that needs a result object takes an `out` parameter — the `*Into` convention throughout `physics/` — and the allocating form stays exported as a thin wrapper for tests and cold callers. When adding a phase to the owner tick, mark it and add its name to `REQUIRED_MARKS`, so deleting the mark is a failure rather than a silent gap.
- **Measure the vehicle before authoring terrain for it.** The ford took four attempts because its depths were reasoned from a guessed ride height. The truck actually rests 1.36 m up with its hull bottom at 0.91 m, intakes at 1.36 / 1.56 / 1.65 / 2.41 m by kind, and buoyancy beats weight past 1.77 m — so "deep water" is a 0.4 m band, not a vibe. Print the numbers from a real `World` first. The same applies to forces: the handbrake spent a long time reusing `brakeForce` (4500 N) against rear tyre grip limits that reach 9826 N, so it could never lock a wheel and the "handbrake" was a mild retarder. **A brake force only means something next to the grip it is fighting.**
- **A test rig has to stay on the terrain.** `handbrake.test.ts` accelerated for 4 s then braked inside a 100 m world — the truck left the heightfield at z=47 and spent three quarters of every measurement window in free fall (y: 1.3 → 0.6 → −2.3 → −7.7), so the numbers it compared were air resistance on a falling body. Any rig that drives in a straight line for seconds needs a world sized for the whole run, and an assertion that the wheels are still on the ground when it reads its result.
...
- Persist ruts across sessions. The shipped field is session-only and lives beside the heightfield rather than in it, so nothing survives a room restart and the coarse Rapier collider still never deforms.
