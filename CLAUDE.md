...
- `packages/shared/src/physics/solidAxleVehicle.ts` — `SolidAxleVehicle`: the owner-authoritative vehicle model. Its fixed phase order captures pose/velocity once, resolves wheel support and compliant carcass/suspension loads in front/rear-left/right order, applies water, advances the engine and physical differential carriers, then resolves relaxed combined-slip tyre and soil forces.
...
1. **`SolidAxleVehicle.preStep()` owns the fixed contact, suspension, water, engine, driveline and tyre-force order.** Keep phase boundaries explicit, retain a single chassis pose/velocity read, and preserve deterministic front/rear then left/right iteration when extracting more of the implementation.
