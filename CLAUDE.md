...
### Shipped
- Mobile HUD layout: one `.hud-dock` column for the bottom-centre instruments, a collapsible transmission panel, and an aux tray split into always-on trail controls plus a toggled pit group. `player-ui.spec.ts` pins the whole panel set as non-overlapping and on-screen across six phone viewports in both panel states.
